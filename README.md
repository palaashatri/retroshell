...
Captured from a Linux VM/Xvfb/Mesa smoke run after the native `wgpu` desktop, interactive menu bar, original desktop icons, managed Finder-style shell windows, focus/raise, active-window close, drag/resize, and visible grow box pass.
...
RetroShell currently builds and launches a native rendered desktop surface, menu strip, desktop icons, app bundle labels, first-party apps wired through RetroKit/RetroSDK, and a first pass at managed shell windows. This implementation is still a foundation, not a polished desktop environment.
...
- `retro-shell` under Linux/Xvfb/Mesa Vulkan llvmpipe, including menu-driven creation of a second managed Finder-style shell window
...
- Window management: focus rings, minimize/zoom/fullscreen controls, modal dialogs, persisted placement, external app surfaces.
