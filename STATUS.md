...
| App | Path | Status |
|-----|------|--------|
| Finder | `apps/finder/` | Prototype: menus, sorted file listing, status/path bar, folder entry, parent/back/forward navigation, and file operation helpers exist; visible navigation controls, real desktop integration, trash UI, contextual menus, and polished multi-window workflows remain incomplete. |
...
- `cargo test --workspace -q` (46 tests)
...
- Linux VM/Xvfb/Mesa Vulkan llvmpipe smoke: `finder` starts against a demo home directory, enters a folder, refreshes path/status display, and captures `docs/screenshots/current-finder.png`.
...
2. Make Finder real: add visible navigation controls, wire file operations to UI, add context menus and trash behavior.
