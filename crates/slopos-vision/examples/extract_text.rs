...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let image_path = args.next().unwrap_or_else(|| "models/vision/ocr_test.png".into());
    ...
    for line in &result.lines {
        let conf = line.confidence.map(|c| format!("{c:.3}")).unwrap_or_else(|| "-".into());
        println!("[{conf}] {}", line.text);
    }
    println!("({} lines, {} words)", result.lines.len(), result.all_words().count());
    Ok(())
}
