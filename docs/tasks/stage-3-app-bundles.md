...
- `slopos-shell` already depends on `serde` (`Cargo.toml:16`) and `toml`
...
- Do not touch the compositor in this stage. Launch integration goes through
  `crates/slopos-shell/src/session_clients.rs` and `launch_services.rs`.
...
- `AppBundle` struct: `crates/slopos-shell/src/launch_services.rs:4-12`
...
- Real spawn: `crates/slopos-shell/src/session_clients.rs:214-235`
...
- App store: single file `apps/appstore/src/main.rs` (~1650 lines).
  ...
  `SLOPOS_APPSTORE_ALLOW_PACKAGE_CHANGES=1`); install button chain
  `main.rs:1025-1028` → `start_install_async` `main.rs:970-989` →
  `backend.install_async` `main.rs:976`. No HTTP/catalog today.
- appstore deps (`apps/appstore/Cargo.toml:10-14`): `slopos-sdk`, `slopos-kit`,
...
1. Confirm the stub and shell-out still exist exactly where the anchors say:
   ```bash
   sed -n '72,118p' crates/slopos-shell/src/launch_services.rs   # hardcoded builtins
...
Precondition:
```bash
cargo build -p slopos-shell 2>&1 | tail -1   # → Finished / no errors
```

Files: Create `crates/slopos-shell/src/bundle.rs`; Modify
`crates/slopos-shell/src/lib.rs` (add `pub mod bundle;`).
...
2. Add `pub mod bundle;` to `crates/slopos-shell/src/lib.rs` (near the other
   `pub mod` lines).
...
- Add a new toml/serde dependency — `slopos-shell` already has both.
...
Precondition:
```bash
cargo test -p slopos-shell bundle:: >/dev/null 2>&1 && echo ok   # Task 3.1 done → ok
```

Files: Modify `crates/slopos-shell/src/launch_services.rs`.
...
Acceptance:
```bash
cargo test -p slopos-shell 2>&1 | grep -E 'test result:' | tail -1
```
→ expect: `test result: ok.` (all slopos-shell tests pass, including the new scan test).
...
Precondition:
```bash
grep -q 'fn spawn_app_client' crates/slopos-shell/src/session_clients.rs && echo ok   # → ok
```

Files: Modify `crates/slopos-shell/src/session_clients.rs`.
...
1. Add a function that, given an `&AppBundle`, builds a `std::process::Command`
   ...
   remove `SLOPOS_LOCK_PASSWORD`), and `spawn()`s it.
...
Acceptance:
```bash
cargo build -p slopos-shell 2>&1 | tail -1 && cargo test -p slopos-shell 2>&1 | grep -E 'test result:' | tail -1
```
...
# e.g. build-app-bundle.sh finder "Finder" com.slopos.finder 0.1.0 /tmp/Applications \
...
Acceptance:
```bash
bash packaging/apps/build-app-bundle.sh finder "Finder" com.slopos.finder 0.1.0 /tmp/rs-apps themes/platinum/icons/finder.png
test -x /tmp/rs-apps/Finder.app/bin/finder && \
grep -q 'bundle_id = "com.slopos.finder"' /tmp/rs-apps/Finder.app/Resources/Info.toml && \
...
1. Call `build-app-bundle.sh` once per app into `${OUTDIR:-/tmp/Applications}`:
   - `finder "Finder" com.slopos.finder` icon `themes/platinum/icons/finder.png`
   - `settings "Settings" com.slopos.settings` icon `themes/platinum/icons/settings.png`
   - `textedit "TextEdit" com.slopos.textedit` icon `themes/platinum/icons/textedit.png`
   - `terminal "Terminal" com.slopos.terminal` icon `themes/platinum/icons/terminal.png`
   - `appstore "App Store" com.slopos.appstore` (no icon — none exists; **CONFIRM
...
Precondition:
```bash
grep -q 'SLOPOS_APPSTORE_ALLOW_PACKAGE_CHANGES' apps/appstore/src/main.rs && echo ok
```
...
2. Delete the `SLOPOS_APPSTORE_ALLOW_PACKAGE_CHANGES` env var and any
...
! grep -q 'SLOPOS_APPSTORE_ALLOW_PACKAGE_CHANGES\|pacman\|apt-get' apps/appstore/src/main.rs && \
...
Steps:
1. Load a catalog at startup: read `SLOPOS_APPSTORE_CATALOG` (a path to a
...
Precondition (on the VM, per [SLOPOS-I.md](../SLOPOS-I.md) §3):
```bash
ssh -i packaging/vm/qa_key -p 2222 retro@127.0.0.1 'cd ~/slopos-i && git pull && cargo build --release --workspace && echo BUILT'
```
→ expect: `BUILT`.
...
1. Build bundles on the VM and stage a catalog:
   ```bash
   ssh ... 'cd ~/slopos-i && OUTDIR=$HOME/Applications bash packaging/apps/build-all-bundles.sh'
   ```
...
3. Add a **rescan hook**: after the store installs, `scan_applications` must pick
   ...
   a signal; otherwise restart `slopos-shell`. (This is the integration Task 3.2/3.3
