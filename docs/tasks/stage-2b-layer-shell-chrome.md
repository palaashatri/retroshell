...
## Why (audit findings)
- slopos-shell renders its whole desktop through **one winit+wgpu xdg-toplevel**
  (`crates/slopos-sdk` `Application`/`WgpuPresenter`). The compositor forces every
...
- The legacy in-window menu bar was already removed (slopos-sdk global-menu-only).
- Font baseline + descenders already fixed (slopos-render/slopos-sdk).
...
## Phases
- **Phase 1 — `LayerSurfacePresenter` in slopos-sdk (or slopos-shell):** a reusable
...
- **Phase 2a — DONE (committed `1bd616c`):** `slopos_sdk::RawSurfaceRenderer`
  (+ pub `Canvas`) wraps `WgpuPresenter::new_raw` so slopos-shell can render onto a
  surface it created. Compiles on the VM.
- **Phase 2b — shell uses one full-output BACKGROUND layer.** slopos-shell maps a
  ...
  - `ShellDesktop` is driven entirely by slopos-sdk's **winit `AppHandler`**
    ...
    input" core today.
  - There are two widget layers: slopos-kit `Widget::draw(&self, &ThemeContext)`
    and the slopos-sdk Canvas draw path (`draw_window`/`draw_desktop_backdrop`).
  - **Plan:** extract the AppHandler render+input core in slopos-sdk into a
    backend-agnostic unit (input event enum in → `render(|canvas| ...)` out), then
    add an sctk-driven driver in slopos-shell that (1) creates the layer surface +
    ...
    whole path behind `SLOPOS_LAYER_SHELL_CHROME` so the winit default is
...
- **Phase 3 — DONE (Env B evidence):** menu bar → `top` layer (exclusive=menu_h),
  ...
  Menu dropdowns paint on Overlay (`slopos-i-menu-popup`); evidence
